...
///////////////////////// TO-DO (3) //////////////////////////////
: _upcCode(other._upcCode), _brandName(other._brandName),
_productName(other._productName), _price(other._price)
...
std::istream & operator>>( std::istream & stream, GroceryItem & groceryItem )
{
  ...
  GroceryItem compGi; //object to compare to input; moved to groceryItem if correct
  char delim = '\0'; //input space and used for comma
  if(stream >> std::ws >> std::quoted(compGi._upcCode) && stream >> std::ws >> delim && delim == ',' && stream >> std::ws >> std::quoted(compGi._brandName) && stream >> std::ws >> delim && delim == ',' && stream >> std::ws >> std::quoted(compGi._productName) && stream >> std::ws >> delim && delim == ',' && stream >> std::ws >> compGi._price )
  {
    groceryItem = std::move(compGi); //moves to working object
  }
